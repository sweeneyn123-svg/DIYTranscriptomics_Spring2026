...
#NS Trying new annotations using an updated version of EnsDb.Hspaiens ----
install.packages(AnnotationHub)
ah <- AnnotationHub()
query(ah, c("EnsDb", "Homo sapiens", "113"))  # 113 is the most recent one available
edb <- ah[["AH119325"]]  # Pick the names() from the query above
Tx_V2 <- transcripts(edb, columns = c("tx_id", "gene_id"))
Tx_V2 <- as_tibble(Tx_V2)
Tx_V2 <- dplyr::rename(Tx_V2, target_id = tx_id)
Tx_V2 <- dplyr::rename(Tx_V2, gene_name = gene_id)
Tx_V2 <- dplyr::select(Tx_V2, "target_id", "gene_name")
#Update, still too many transcripts missing from tx2gene (122001) because too new of version
#NS Trying another work around to reduce the number of missing transcripts ---- 
install.packages("Biostrings")
library(Biostrings)
fasta_file <- "~/Desktop/DIYTranscriptomics/course_dataset/kallisto/Homo_sapiens.GRCh38.cdna.all.fa"
fasta <- readDNAStringSet(fasta_file)
headers <- names(fasta)
# FIXED: Extract transcript ID (first word only)
target_id <- sub("^(ENST[0-9]+\\.[0-9]+).*", "\\1", headers)
# FIXED: Extract gene symbol (not ENSG)
gene_name <- sub(".*gene_symbol:([A-Za-z0-9\\-]+).*", "\\1", headers)
gene_name[gene_name == headers] <- NA #Cleans up where no gene name found
Tx_V3 <- data.frame(target_id = target_id, gene_name = gene_name, stringsAsFactors = FALSE)
Tx_V3 <- Tx_V3[!is.na(Tx_V3$gene_name) & Tx_V3$gene_name != "", ] #Filters if gene_name is NA or empty
Tx_V3 <- unique(Tx_V3) #removes any duplicates (may not be necessary)
...
#Nick's Version of the Essentials ----
library(tidyverse) # provides access to Hadley Wickham's collection of R packages for data science, which we will use throughout the course
library(tximport) # package for getting Kallisto results into R
library(ensembldb) #helps deal with ensembl
library(Biostrings)
